...
#standard velocity verlet scheme: new positions
def new_pos_euler(Particle):
    '''
    if (dyn_friction_on == 1):
        dynamic_frictions(Particle)
   #     expo_frictions(Particle)
        Particle.counter = Particle.counter + 1
        if ((Particle.counter % 1000000) == 0):
            print("counter:")
            print(Particle.counter)
 ...
 #   Particle.position = Particle.position + Particle.velocity*timestep + (Particle.force )*(timestep**2)*0.5 / Particle.mass
    ...
    actforces = new_forces_thermostat_euler(Particle)  

   # Particle.velocity = timestep * actforces 

    
    Particle.position = Particle.position + timestep * actforces / Particle.friction
...
#updating of the acting forces
def new_forces_thermostat_euler(Particle):
    force = 0
   # force = force - Particle.friction * Particle.velocity * Particle.mass
    if (harmpot_on == 1):
...
#standard velocity verlet scheme: new velocities
def new_vel_euler(Particle):

  #  actforces = new_forces_thermostat(Particle)  

  #  Particle.velocity = Particle.velocity + timestep * (actforces + Particle.force) / (2*Particle.mass )
   # Particle.velocity = Particle.velocity
    Particle.velocity = (Particle.position - Particle.positions[-1])/timestep





def run_simulation_euler(particles):
    # loop for 2 (or more) particles  
    for step in range(steps):
        for Particle in particles:        
            # new positions
            new_pos_euler(Particle)
        for Particle in particles:  
            # new velocities
            new_vel_euler(Particle)
...
def init_particles(temp2 = 5):
    ...
    #1d
    particle1 = Particle(np.array([1,2,-1]),np.array([0,0,0]),1,np.array([setfriction,setfriction,setfriction]),np.array([0,0,0]),1)
    particle2 = Particle(np.array([5,-2,3]),np.array([0,0,0]),1,np.array([setfriction,setfriction,setfriction]),np.array([0,0,0]),1+temp2)
    particle3 = Particle(np.array([3,-1,2]),np.array([0,0,0]),1,np.array([setfriction,setfriction,setfriction]),np.array([0,0,0]),temp2*2)

    #3d
    #particle5 = Particle(np.array([1,2,-1]),np.array([0,0,0]),1,np.array([setfriction,setfriction,setfriction]),np.array([0,0,0]),1)
    #particle6 = Particle(np.array([5,-2,3]),np.array([0,0,0]),1,np.array([setfriction,setfriction,setfriction]),np.array([0,0,0]),temp2)

    partdim = len(particle1.position)
